...
public class TipoAtraccionDao {
    ...
    public List<TipoAtraccion> getTiposAtracciones(TipoAtraccion tipoAtraccion) {
        ...
        try (Session session = HibernateUtil.getSessionFactory().openSession()) {
            return (List<TipoAtraccion>) session
                    .createQuery("FROM TipoAtraccion TA")
                    .getResultList();
        }catch(Exception e) {
            e.printStackTrace();
        }
    ...
    public TipoAtraccion unTipoAtraccion(Usuario usuario){
        ...
        try(Session session = HibernateUtil.getSessionFactory().openSession()){
            //la condicion que coloque solo es una prueba podemos filtrar por lo mismo de arriba, presupuesto, tiempo y demas
            return miTipoAtraccion = (TipoAtraccion) session.createQuery("FROM TipoAtraccion TA where TA.id < :preferencia")
                    .setParameter("preferencia",usuario.getFav())
                    .getSingleResult();
        }catch (Exception e){
            e.printStackTrace();
        }
    ...
    public boolean baja(TipoAtraccion tipoAtraccion){
        ...
        try(Session session = HibernateUtil.getSessionFactory().openSession()){
            ...
            tipoAtraccion1 = (TipoAtraccion) session.createQuery("from TipoAtraccion TA where TA.id = :tipo_id")
                    .setParameter("tipo_id", tipoAtraccion.getId())
                    .getSingleResult();
    ...
    // mostramos los valores de un usuario getUno(id), ahi se podran modificar algunos campos
    //  y lo vamos a guardar en un usuario temporal, lo comparamos y si son distintos le hacemos update
